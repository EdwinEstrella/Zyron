...
CREATE POLICY "project_admin_policy" ON public.accounting_accounts FOR ALL TO project_admin, service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "project_admin_policy" ON public.accounting_journal_entries;
CREATE POLICY "project_admin_policy" ON public.accounting_journal_entries FOR ALL TO project_admin, service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "project_admin_policy" ON public.accounting_journal_lines;
CREATE POLICY "project_admin_policy" ON public.accounting_journal_lines FOR ALL TO project_admin, service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "project_admin_policy" ON public.accounting_posting_rules;
CREATE POLICY "project_admin_policy" ON public.accounting_posting_rules FOR ALL TO project_admin, service_role USING (true) WITH CHECK (true);

-- Políticas multi-inquilino de Zyron para Contabilidad
...
CREATE POLICY "tenant_accounting_accounts_read" ON public.accounting_accounts
  ...
  USING (tenant_id IN (SELECT get_user_tenants()) OR public.is_super_admin());
...
CREATE POLICY "tenant_accounting_accounts_write" ON public.accounting_accounts
  ...
  USING (tenant_id IN (SELECT get_user_tenants()) OR public.is_super_admin())
  WITH CHECK (tenant_id IN (SELECT get_user_tenants()) OR public.is_super_admin());
